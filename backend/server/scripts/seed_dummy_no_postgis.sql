-- Seed dummy data (No PostGIS variant)
-- Pre-req: run init_no_postgis.sql
...
with citizen as (
  ...
  values
  ('open',         (select cid from citizen), 'Hyderabad, TS', 'medium', null,                     null,                      'Pothole on main road near signal', null, '{"type":"Point","coordinates":[78.4867,17.3850]}'),
  ('assigned',     (select cid from citizen), 'Hyderabad, TS', 'high',   (select sid from staff), now() + interval '2 days', 'Garbage pile-up in colony',         null, '{"type":"Point","coordinates":[78.4900,17.3900]}'),
  ('in_progress',  (select cid from citizen), 'Hyderabad, TS', 'medium', (select sid from staff), now() + interval '1 day',  'Street light not working',          null, '{"type":"Point","coordinates":[78.4850,17.3800]}'),
  ('resolved',     (select cid from citizen), 'Hyderabad, TS', 'low',    (select sid from staff), now() - interval '1 hour', 'Water leakage in lane',             null, '{"type":"Point","coordinates":[78.4800,17.3700]}'),
  ('closed',       (select cid from citizen), 'Hyderabad, TS', 'low',    null,                     null,                      'Stray dogs issue',                  null, '{"type":"Point","coordinates":[78.4700,17.3600]}')
...
insert into ticket_updates(ticket_id, message, created_by)
select t.id, 'Assigned to staff', (select sid from staff)
