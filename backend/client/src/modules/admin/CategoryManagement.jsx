...
import { categoriesAPI, departmentsAPI } from '../../services/api.js'
...
export default function CategoryManagement() {
  ...
  const handleDelete = async (categoryId) => {
    if (window.confirm('Are you sure you want to delete this category?')) {
      try {
        await categoriesAPI.delete(categoryId, token)
        loadData()
      } catch (err) {
        setError(err.message)
      }
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Manage issue categories and their hierarchy</p>
        </div>
        <button
          onClick={() => {
            resetForm()
                  ...
                  (categories || []).map((category) => (
                    <tr key={category.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {departments.find(d => d.id === category.department_id)?.name || 'No department'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {category.parent_id ? 
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                          category.is_active !== false ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                        }`}>
